...
export class VanillaMachine<T extends MachineSchema> {
  ...
  constructor(
    ...
    const state = bindable(() => ({
      ...
      onChange: (nextState, prevState) => {
        ...
        if (cleanup)
          this.effects.set(nextState as string, cleanup)
        ...
        if (prevState === INIT_STATE) {
          ...
          if (cleanup)
            this.effects.set(INIT_STATE, cleanup)
        }
  ...
  private choose: ChooseFn<T> = (transitions) => {
    return toArray(transitions).find((t: any) => {
      let result = !t.guard
      if (isString(t.guard))
        result = !!this.guard(t.guard)
      else if (isFunction(t.guard))
        result = t.guard(this.getParams())
      return result
    })
  }
